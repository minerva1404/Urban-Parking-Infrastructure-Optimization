...
**Project Overview**  
Real-time HDB carpark streaming pipeline with Kafka and Spark, transforming raw government data into analytics-ready insights for urban infrastructure optimization. Simulates production-style smart city data workflows, demonstrating end-to-end ETL and dashboarding skills.  
...
## 🛠️ Tech Stack
- **Python 3.9+** – main programming language for producer, consumer, and Spark jobs  
- **Apache Kafka** – real-time event streaming (Bronze layer)  
- **Apache Spark Structured Streaming 3.5+** – Silver & Gold layers, ETL + aggregations  
- **JSON & Parquet** – storage formats for Silver and Gold layers  
- **Power BI** – visualization of aggregated KPIs & infrastructure insights  
- **Git** – version control and repo management  
- **Java 8/11** – required for running Spark & Kafka
...
data.gov.sg API ↓ Kafka Producer ↓ Kafka Topic (Bronze) ↓ Spark Structured Streaming (Silver) ↓ Cleaned JSON Storage ↓ Spark Structured Streaming (Gold) ↓ Aggregated Parquet (Analytics-Ready)
...
(avg_decks * 10) + (avg_gantry_height * 15) + (night_parking_count * 2) + (free_parking_count * 2)
